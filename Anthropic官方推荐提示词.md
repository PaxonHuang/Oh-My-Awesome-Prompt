...
```Plain
我需要开发 [功能描述]。请按照以下步骤：1. 先阅读相关代码了解现有架构2. 制定详细的实现计划3. 实现核心功能4. 编写测试5. 更新文档每完成一步都要暂停等待我确认。
```
...
```Plain
请重构 [文件名] 中的 [函数/类] ，目标是：- 提高代码可读性- 减少重复代码- 遵循最佳实践- 保持功能不变请先分析现有代码，然后提供重构计划。
```
...
```Plain
请为项目配置 CI/CD 流程，包括：- 自动化测试- 代码质量检查- 自动部署- 通知机制
```

### Docker 配置  

```Plain
请为项目创建 Docker 配置，包括：- Dockerfile- docker-compose.yml- 环境变量配置- 部署说明
```
...
1. **使用 @ 引用文件**：`@src/components/Button.tsx`
2. **利用扩展思考**：按两次 Shift+TAB 进入 PLAN 模式
3. **合理清理上下文**：使用 `/clear` 和 `/compact`
4. **创建自定义命令**：在 `.claude/commands/` 目录下
5. **批量操作**：一次性处理多个相似任务
